...
mod tests {
    ...
    async fn poll_git_emits_branch_and_commit_events_for_linked_worktree() {
        ...
        let actual_root =
            std::path::PathBuf::from(branch_event.payload["repo_path"].as_str().unwrap())
                .canonicalize()
                .unwrap();
        let actual_worktree =
            std::path::PathBuf::from(branch_event.payload["worktree_path"].as_str().unwrap())
                .canonicalize()
                .unwrap();
        ...
        let commit_root =
            std::path::PathBuf::from(commit_event.payload["repo_path"].as_str().unwrap())
                .canonicalize()
                .unwrap();
        let commit_worktree =
            std::path::PathBuf::from(commit_event.payload["worktree_path"].as_str().unwrap())
                .canonicalize()
                .unwrap();
        assert_eq!(commit_root, expected_root);
        assert_eq!(commit_worktree, expected_worktree);
