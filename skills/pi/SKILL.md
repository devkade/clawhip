...
`create.sh` launches Pi in a clawhip-monitored tmux session. The primary goal is to keep the session directly visible and attachable while clawhip handles keyword/stale alerts and route delivery. The wrapper also emits lightweight `session.started`, `session.finished`, and `session.failed` signals as a secondary notification layer. The launcher prefers a globally installed `pi` on `PATH`; if that is unavailable, it falls back to a local `pi-mono` checkout by looking for `pi-test.sh`, unless you explicitly set `CLAWHIP_PI_BIN`. It also auto-detects `clawhip` itself and falls back to repo-local `cargo run -- ...` when the binary is not installed on `PATH`, and it avoids shell-specific rc sourcing inside the tmux bootstrap script. If you pass a prompt, the script waits for the configured delay, then sends the prompt via `tmux send-keys -l` before pressing Enter.
...
| `CLAWHIP_PI_BIN` | auto-detected (`pi` on `PATH`, otherwise `pi-test.sh` inside `pi-mono`) | Override the Pi launcher explicitly |
| `CLAWHIP_PI_PROMPT_DELAY` | `10` | Seconds to wait before sending the initial prompt into tmux |
| `CLAWHIP_BIN` | auto-detected (`clawhip` on `PATH`, otherwise repo-local `cargo run -- ...`) | Override the clawhip launcher explicitly |
